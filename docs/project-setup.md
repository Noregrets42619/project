...
基于项目本身，由于开发板自带功放和音频信号解码元件，只需购买不超过功放带载能力的小型喇叭模块即可。

屏幕模块，项目本身使用`I80`协议的`320*190`尺寸屏幕，硬件虽然支持`MIPI`屏幕，但由于性能和屏幕素质要求极低，并且价格昂贵，遂选用4线SPI协议进行屏幕移植，将`I80`协议用SPI重写，并移植至idf环境。

外设键盘，选用支持`MIDI`协议的打击键盘，可以输出打击力度，使音量能够根据按键力度而变化。
